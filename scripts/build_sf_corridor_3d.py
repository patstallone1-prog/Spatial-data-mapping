...
function pavementRunsOutsideCarriageway(points, width) {
  ...
  for (let i = 1; i < points.length; i += 1) {
    ...
    for (let k = i === 1 ? 0 : 1; k <= steps; k += 1) {
      ...
      const overlaps = insideCarriageway(x, z, 0.30) || (innerEdge && outerEdge);
      if (overlaps) {
        finish();
      } else {
...
function walkFitsAt(x, z, nx, nz, width) {
  ...
  const innerEdge = insideCarriageway(x + nx * edge, z + nz * edge, 0.30);
  const outerEdge = insideCarriageway(x - nx * edge, z - nz * edge, 0.30);
  return !(insideCarriageway(x, z, 0.30) || (innerEdge && outerEdge));
}
...
const surfaceKind = isCrossing ? ((way.continental || way.alley_mouth) ? "crossing" : "crossing_edges")
  ...
  if (isSidewalk || isPath) {
    ...
    if (isCrossing) {
      ...
      addCrossingTactilePads(surfacePoints, widthMeters, roadTop + KERB + 0.006);
    }
  }
