...
function entryTexture(seed, archetype) {
  ...
  if (archetype === "retail" || archetype === "restaurant") {
    const awning = archetype === "restaurant"
      ? (random(seed + 5) < 0.5 ? "#9b2f24" : "#1f6b55")
      : (random(seed + 5) < 0.5 ? "#264f7f" : "#b48a35");
    ctx.fillStyle = "rgba(18,22,24,0.82)";
    ctx.fillRect(8, 36, 112, 70);
    const glass = ctx.createLinearGradient(0, 42, 0, 108);
    glass.addColorStop(0, "rgba(140,174,192,0.88)");
    glass.addColorStop(1, "rgba(64,84,96,0.92)");
    ctx.fillStyle = glass;
    ctx.fillRect(13, 42, 42, 58);
    ctx.fillRect(73, 42, 42, 58);
    ctx.fillStyle = "rgba(36,28,22,0.95)";
    ctx.fillRect(57, 43, 14, 60);
    ctx.fillStyle = "rgba(255,232,160,0.74)";
    ...
    ctx.strokeRect(12.5, 41.5, 103, 59);
  } else if (archetype === "office" || archetype === "hotel") {
    const glass = ctx.createLinearGradient(0, 18, 0, 112);
    glass.addColorStop(0, "rgba(160,192,206,0.88)");
    glass.addColorStop(1, "rgba(70,90,102,0.92)");
    ctx.fillStyle = "rgba(0,0,0,0.28)";
    ctx.fillRect(16, 17, 96, 94);
    ctx.fillStyle = glass;
    ctx.fillRect(20, 20, 88, 88);
    ctx.fillStyle = "rgba(255,255,255,0.52)";
    ctx.fillRect(62, 20, 3, 88);
    ctx.fillRect(20, 55, 88, 2);
    ctx.fillStyle = "rgba(20,24,26,0.88)";
    ctx.fillRect(49, 70, 30, 38);
    if (archetype === "hotel") {
      ctx.fillStyle = "rgba(255,255,255,0.76)";
      ctx.font = "bold 12px system-ui, sans-serif";
      ctx.textAlign = "center";
      ctx.fillText("HOTEL", 64, 36);
    }
  } else {
...
function addBuildingNamePlaque(group, feature, seed, height, tint) {
  ...
  const slot = signSlot(name, trade);
...
function addGroundFacadeDetail(group, feature, seed, height) {
  ...
  if (!shopfront && !residential) return;
  const wall = bestFacadeWall(feature, shopfront ? false : true);
  if (!wall) return;
  const panelW = shopfront ? Math.min(wall.span * 0.86, 18) : Math.min(wall.span * 0.62, 7.2);
  const panelH = shopfront ? Math.min(4.2, Math.max(3.1, height * 0.34)) : Math.min(3.35, height - 0.3);
...
function grassTexture(kind = "yard", variant = 0) {
  ...
  // Strands, not a wash. This was a field of single pixels at 40% opacity over a flat green,
  // tiled every four metres, and single pixels at low opacity average out -- from anything
  // further than a few metres it was one continuous sheet of pale green with no grain in it at
  // all. Grass reads as grass because it is thousands of separate blades lying in slightly
  // different directions and catching the light differently, so these are drawn as short
  // strokes with real contrast between them and a base dark enough to survive the exposure.
  const size = 384;
  const canvas = document.createElement("canvas");
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext("2d");
  const parkPalettes = [
    ["#36a846", "rgba(20,92,29,0.42)", "rgba(111,202,100,0.30)", "rgba(210,239,170,0.08)"],
    ["#2f9741", "rgba(18,83,33,0.45)", "rgba(135,211,103,0.26)", "rgba(232,238,164,0.06)"],
    ["#409d35", "rgba(27,86,28,0.38)", "rgba(154,213,118,0.24)", "rgba(196,232,151,0.07)"],
    ["#2d8f4a", "rgba(17,74,45,0.40)", "rgba(102,192,128,0.22)", "rgba(213,234,180,0.055)"],
  ];
  const yardPalettes = [
    ["#2f9738", "rgba(21,80,24,0.46)", "rgba(92,178,78,0.26)", "rgba(220,231,150,0.055)"],
    ["#3a8d35", "rgba(31,78,25,0.44)", "rgba(126,180,79,0.23)", "rgba(213,211,136,0.05)"],
    ["#287f3d", "rgba(17,68,32,0.48)", "rgba(83,158,89,0.25)", "rgba(200,225,158,0.055)"],
    ["#468d31", "rgba(45,76,22,0.40)", "rgba(145,180,73,0.22)", "rgba(226,219,145,0.05)"],
    ["#2f8850", "rgba(22,76,48,0.42)", "rgba(86,166,117,0.22)", "rgba(194,229,176,0.055)"],
  ];
  const palettes = kind === "park" ? parkPalettes : yardPalettes;
  const palette = palettes[Math.abs(variant) % palettes.length];
  const [base, dark, light, mow] = palette;
  const seed = (variant + 1) * 997 + kind.length * 131;
  ctx.fillStyle = base;
  ctx.fillRect(0, 0, size, size);

  // Patchiness first: the underlying unevenness of a lawn, so the blades are not scattered over
  // a uniform ground. A few straw/dirt flecks keep small residential yards from reading as
  // bright carpet; parks are cleaner and a touch lighter.
  for (let i = 0; i < 130; i += 1) {
    const r = 8 + random(seed + i * 17) * (variant % 2 ? 72 : 48);
    ctx.fillStyle = random(seed + i * 5) < 0.55 ? dark : light;
    ctx.beginPath();
    ctx.arc(random(seed + i * 3) * size, random(seed + i * 7) * size, r, 0, Math.PI * 2);
    ctx.fill();
  }
  for (let i = 0; i < (kind === "park" ? 260 : 520); i += 1) {
    const x = random(seed + i * 19.1 + 11) * size;
    const y = random(seed + i * 23.7 + 5) * size;
    const w = 0.5 + random(seed + i * 3.2) * 1.8;
    const h = 0.5 + random(seed + i * 4.6) * 1.6;
    ctx.fillStyle = random(seed + i * 13.7) < 0.55 ? "rgba(117,92,48,0.20)" : "rgba(221,204,125,0.13)";
    ctx.fillRect(x, y, w, h);
  }

  for (let i = 0; i < 38; i += 1) {
    const x = random(seed + i * 31.7) * size;
    const y = random(seed + i * 41.3) * size;
    const rx = 14 + random(seed + i * 2.1) * 42;
    const ry = 4 + random(seed + i * 5.9) * 12;
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate((random(seed + i * 9.7) - 0.5) * Math.PI);
    ctx.fillStyle = random(seed + i * 12.1) < 0.5 ? "rgba(236,214,132,0.075)" : "rgba(16,64,24,0.10)";
    ctx.beginPath();
  ...
  const passes = [
    ["rgba(12,68,20,0.88)", 7600, 4.2, 1.00],
    ["rgba(31,111,35,0.82)", 6800, 3.7, 0.88],
    ["rgba(78,168,65,0.72)", 5200, 3.2, 0.70],
    ["rgba(158,220,130,0.52)", 2600, 2.8, 0.45],
  ];
  let n = 0;
  for (const [colour, count, len, opacityJitter] of passes) {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const sweep = kind === "park" ? 0.95 : 1.6;
      const angle = (random(seed + n * 7.3) - 0.5) * sweep - Math.PI / 2 + (variant - 2) * 0.12;
      const l = len + random(seed + n * 11.9) * len;
      ctx.globalAlpha = 0.65 + random(seed + n * 2.6) * opacityJitter * 0.35;
      ctx.beginPath();
      ctx.moveTo(x, y);
  ...
  ctx.globalAlpha = 1;
  // Low, soft mowing direction, not stripes. It breaks repetition over large parks without
  // declaring an exact maintenance pattern the source data does not know.
  ctx.strokeStyle = mow;
  ctx.lineWidth = variant % 3 === 0 ? 2 : 1.35;
  const mowStep = 28 + (variant % 5) * 9;
  const mowLean = 0.25 + (variant % 4) * 0.13;
  for (let i = -size; i < size * 2; i += mowStep) {
    ctx.beginPath();
    ctx.moveTo(i, 0);
    ctx.lineTo(i + size * mowLean, size);
    ctx.stroke();
  }
