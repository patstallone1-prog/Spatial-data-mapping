...
function nominalRoadWidth(way) {
  ...
  const measured = way.road_source === "curb_geometry";
...
function recentreDividedHalf(way) {
  // The outer kerb is the one the footway is on; the way is moved so that its outer edge lies
  // on it, vertex by vertex. One offset for the whole way is the rule for a street between
  // two kerbs, and it is right there because the kerbs are parallel; a divided half's outer
  // kerb is not parallel to the median it was drawn along -- Broadway's westbound half widens
  // by five metres over its last seventy toward the tunnel -- so each vertex takes the offset
  // of the stations around it, where they agree.
  const side = way.walk_sides[0];
  const width = dividedHalfWidth(way);
  const points = way.points;
  const stations = [];
  const dense = densifyWay(points, RECENTRE_STATION_M);
  const length = wayLength(points);
  if (length < RECENTRE_END_MARGIN_M * 2 + RECENTRE_STATION_M) return false;
  let along = 0;
  for (let i = 1; i < dense.length - 1; i += 1) {
    ...
    if (along < RECENTRE_END_MARGIN_M || along > length - RECENTRE_END_MARGIN_M) continue;
    const dx = qx - px;
    const dz = -(qy - py);
    const span = Math.hypot(dx, dz) || 1;
    const reach = Math.min(30, width * 1.3 + 5);
    const nx = dz / span, nz = -dx / span;
    const hits = rayCurbIntersections(x, -y, nx, nz, reach, officialCurbGrid);
    const kerb = side > 0 ? hits.find((v) => v > 1.2)
      : hits.filter((v) => v < -1.2).pop();
    if (kerb === undefined) continue;
    // The median's own kerb, on the other side: the city maps its islands, and between the
    // outer kerb and the island kerb is the whole of this half's carriageway -- which is
    // wider than its lanes where a turn pocket opens, and Broadway's westbound half runs
    // from 8.5 m at Polk to 11.7 m by Larkin. Without an island kerb the lanes say.
    const islands = rayCurbIntersections(x, -y, nx, nz, Math.abs(kerb) + 2, officialIslandCurbGrid);
    const island = side > 0 ? islands.filter((v) => v < kerb - 2.5 && v > -12).pop()
      : islands.find((v) => v > kerb + 2.5 && v < 12);
    if (island !== undefined) {
      stations.push([along, (kerb + island) / 2, Math.abs(kerb - island)]);
    } else {
      stations.push([along, kerb - side * width / 2, width]);
    }
  }
  ...
  for (let j = 0; j < points.length; j += 1) {
    ...
    const agreed = spread <= RECENTRE_MAX_SPREAD_M;
  ...
  if (!offsets.some((o) => o !== null)) return false;
  // A vertex with no width of its own carries its nearest neighbour's.
  ...
  way._spans = spans;
  // A vertex with no stations of its own carries its nearest neighbour's offset.
  ...
  way._node = [points[0].slice(), points[points.length - 1].slice()];
  way._shift = [offsets[0], offsets[offsets.length - 1]];
  if (offsets.some((o) => Math.abs(o) >= RECENTRE_MIN_SHIFT_M)) {
    way.points = offsetWay(points, offsets);
    way.recentred_m = +Math.max(...offsets.map(Math.abs)).toFixed(3);
  }
  const sorted = spans.map(([, w]) => w).sort((p, q) => p - q);
  way.road_m = +sorted[Math.floor(sorted.length / 2)].toFixed(3);
  way.road_source = "divided_half";
  return true;
}

//: The half road width at a distance along a way: from its per-vertex spans where the city's
//: kerbs gave it some, else the one width it is drawn at everywhere.
...
function recentreStreetsOnOfficialKerbs(ways) {
  if (!officialCurbGrid.size) return { moved: 0, widened: 0 };
  let moved = 0;
  let widened = 0;
  for (const way of ways) {
    if (way.kind !== "street" || !way.points || way.points.length < 2) continue;
    ...
    if (isDividedHalf(way)) {
      if (recentreDividedHalf(way)) { moved += 1; widened += 1; }
      continue;
    }
    const road = way.road_m || 8.0;
    const dense = densifyWay(way.points, RECENTRE_STATION_M);
    const length = wayLength(way.points);
    if (length < RECENTRE_END_MARGIN_M * 2 + RECENTRE_STATION_M) continue;
    const offsets = [];
    const spans = [];
    let along = 0;
    for (let i = 1; i < dense.length - 1; i += 1) {
      ...
      const found = officialKerbOffsetsAt(x, -y, dz / span, -dx / span, road);
      if (!found) continue;
      offsets.push(found.offset);
      spans.push(found.span);
    }
    if (offsets.length < 3) continue;
    const sorted = offsets.slice().sort((p, q) => p - q);
    const median = sorted[Math.floor(sorted.length / 2)];
    const spread = sorted[Math.floor(sorted.length * 0.75)] - sorted[Math.floor(sorted.length * 0.25)];
    if (spread > RECENTRE_MAX_SPREAD_M || Math.abs(median) > RECENTRE_MAX_SHIFT_M) continue;
    // Where the way stood, kept for the junction it shares with the ways that meet it there.
    way._node = [way.points[0].slice(), way.points[way.points.length - 1].slice()];
    way._shift = [median, median];
    if (Math.abs(median) >= RECENTRE_MIN_SHIFT_M) {
      way.points = offsetWay(way.points, median);
      way.recentred_m = +median.toFixed(3);
      moved += 1;
    }
    if (way.road_source !== "curb_geometry") {
      const widths = spans.slice().sort((p, q) => p - q);
      way.road_m = +widths[Math.floor(widths.length / 2)].toFixed(3);
...
//: A way too short to be recentred on its own -- OpenStreetMap splits a street at every
//: driveway node and change of tagging, and Polk Street leaves Broadway as a 23 m piece --
//: moves with the street it is part of: the same shift as the way that carries on from its
//: end. Left where it was, it stood two metres off the pieces either side of it, with a jog in
//: the kerb at each node and its pavement laid across the roadway on one side.
...
function indexBulbOuts(ways) {
  ...
  for (const way of ways) {
    ...
    for (let i = 0; i < dense.length; i += 1) {
      ...
      const span = Math.hypot(dx, dz) || 1;
      // The left normal, in the x/z frame the kerb grid is indexed in.
      const hits = rayCurbIntersections(x, -y, dz / span, -dx / span, inner + 0.6, officialCurbGrid);
      for (const side of [1, -1]) {
        const kerb = side > 0 ? hits.find((v) => v > 1.2) : hits.filter((v) => v < -1.2).pop();
        let depth = kerb === undefined ? 0 : inner - Math.abs(kerb);
...
function addKerbsidePavement(renderPoints, side, inner, walk, color, opacity, streetY,
  ...
  const kerbAt = [];
  {
    ...
    for (let i = 0; i < spine.length; i += 1) {
      ...
      // Where the city's kerbs put this half wider than its lanes, the kerb line is there;
      // never nearer than the drawn width, since the road under it is drawn at that.
      const here = way && way._spans ? Math.max(inner, halfWidthAt(way, along)) : inner;
      kerbAt.push(here);
  ...
  for (let i = 0; i < spine.length; i += 1) {
    ...
    const nx = -dz / span;
    const nz = dx / span;
    const blocked = kerbsideBlockedAt(before, spine[i], after, side, inner);
    ...
    meets.push(blocked ? null : mappedWalkBeyondKerb(sx, -sy, -side * nx, -side * nz, inner));
  }
  ...
  const emit = (from, to) => {
    ...
    if (met) {
      if (typeof group[from] === "string" && group[from].startsWith("gap:")) {
        let gap = 0;
        for (let i = from; i <= to; i += 1) gap = Math.max(gap, meets[i] ? meets[i].edge - inner : 0);
...
window.kerbside = {
  ...
  officialIslandCurbGrid, isDividedHalf, roadwayCovered, roadFillGrid, filledAt, cornerLegs, cornerLegsAt, cornerNeighbour, bulbDepthAt, legBulb, cornerQuad, pavementCornerCuts,
