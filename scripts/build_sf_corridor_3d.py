...
function addKerbsidePavement(renderPoints, side, inner, walk, color, opacity, streetY,
  ...
  for (let i = 0; i < spine.length; i += 1) {
    ...
    for (let w = 0; w < widths.length; w += 1) {
      const centre = offsetWay([before, spine[i], after], side * (kerbAt[i] + widths[w] / 2))[1];
      const [cx, cy] = xy(centre[0], centre[1]);
      fits[w].push(!blocked && walkFitsAt(cx, -cy, nx, nz, widths[w]));
    }
    // The mapped footway on this side, if OpenStreetMap has one here. (nx, nz) is the right
    // normal of travel in the x/z frame, and `side` counts left as positive.
  ...
  recordEnd(cutStart, 0, 1, side, chosen[0] >= 0 ? widths[chosen[0]] : 0);
  recordEnd(cutEnd, n - 1, n - 2, -side,
            chosen[chosen.length - 1] >= 0 ? widths[chosen[chosen.length - 1]] : 0);
...
function fillRoadwayToKerbs() {
  ...
  for (const way of DATA.ways) {
    ...
    for (let i = 0; i < dense.length; i += 1) {
      ...
      if (neg === undefined || pos === undefined || pos - neg > ROAD_FILL_MAX_SPAN_M) continue;
      const islands = rayCurbIntersections(x, z, nx, nz, 22, officialIslandCurbGrid)
        .filter((v) => v > neg && v < pos);
      // The half-station either side along the way that this cross-section stands for.
      ...
      for (const [t0, t1] of runs) {
        ...
        const ring = [at(t0, -halfStep), at(t1, -halfStep), at(t1, halfStep), at(t0, halfStep)];
        // Island kerbs at both edges of the run make it an island.
        const kerbed = (t) => islands.some((v) => Math.abs(v - t) <= 0.45);
        if (width >= ROAD_FILL_ISLAND_MIN_M && kerbed(t0) && kerbed(t1)) {
          const top = ROAD_TOP_M + MEDIAN_KERB_M;
          quad(island, ring, top, 4);
          face(island, ring[0], ring[3], top, ROAD_TOP_M);
          face(island, ring[1], ring[2], top, ROAD_TOP_M);
          counts.islands += 1;
        } else {
          ...
          if (left && right && left.source !== right.source && width <= 3.5) {
            ...
            const sink = opposing ? lines.yellow : lines.white;
            const offsets = opposing ? [-(MARK_W + DOUBLE_GAP_M) / 2, (MARK_W + DOUBLE_GAP_M) / 2] : [0];
            for (const off of offsets) sink.push([at(mid + off, -halfStep), at(mid + off, halfStep)]);
            counts.lines += 1;
          }
        }
        stampFill(...at(t0, 0), ...at(t1, 0));
      }
    }
  }
