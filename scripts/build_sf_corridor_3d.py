...
def fold_parking_aisles_into_lots(ways: list[dict[str, Any]]) -> int:
    ...
    folded = 0
    for way in ways:
        if way.get("kind") != "street" or way.get("service") != "parking_aisle":
            continue
        points = way.get("points") or []
        if len(points) < 2:
            continue
...
function officialCurbCrossingSpan(x, z, dx, dz, expectedHalfWidth) {
  if (!officialCurbGrid.size) return null;
  const reach = Math.min(32.0, Math.max(12.0, expectedHalfWidth * 2.8));
...
function crossingRoadSpanPoints(points) {
  ...
  const curbSpan = officialCurbCrossingSpan(best.x, best.z, ux, uz, best.half);
...
function addBikeLaneMarkings(way, renderPoints, roadWidth, roadTop) {
  ...
  for (const side of sides) {
    ...
    const lane = densifyWay(trimWayEnds(centre, cutStart, cutEnd), 2.0);
...
//: parking aisle outside any mapped lot is two, so cars pass. Alleys keep the street default.
...
function cornerNeighbour(leg, side) {
  ...
  // Two legs of one street forking apart are a divided road, not a crossroads: the wedge
  // between Columbus's two carriageways is a gore in the roadway, not a pavement corner.
...
function cornerCutFor(A, side) {
  ...
  const [tK, uK] = frame.meet(A.inner, B.inner);
  const [tO, uO] = frame.meet(A.inner + A.walk, B.inner + B.walk);
...
function reconcilePavementCorners() {
  ...
  for (const leg of cornerLegs) {
    for (const side of [1, -1]) {
      ...
      const [tKl, uKl] = corner.frame.meet(leg.inner + lip, corner.B.inner + lip);
      const cuts = corner.frame.meetCuts(corner.cut, other.cut);
      if (!cuts) continue;
      const [vC, wC] = cuts;
      if (!(corner.cut > tKl + 0.05 && other.cut > uKl + 0.05)) continue;
      if (!(vC > leg.inner + lip + 0.2 && wC > corner.B.inner + lip + 0.2)) continue;
      corner.valid = true;
    }
  }
  let valid = 0;
...
function cornerQuad(leg, side) {
  ...
  const lip = KERB_LIP_M;
  const [tK] = frame.meet(leg.inner + lip, B.inner + lip);
  const cuts = frame.meetCuts(cutA, cutB);
  if (!cuts) return null;
  const [vC] = cuts;
  const K = frame.alongA(tK, leg.inner + lip);
  const A1 = frame.alongA(cutA, leg.inner + lip);
  const C = frame.alongA(cutA, vC);
  const B1 = frame.alongB(cutB, B.inner + lip);
  const [tk2] = frame.meet(leg.inner + lip / 2, B.inner + lip / 2);
  const kerb = [frame.alongA(cutA, leg.inner + lip / 2), frame.alongA(tk2, leg.inner + lip / 2),
                frame.alongB(cutB, B.inner + lip / 2)];
...
function addKerbsidePavement(renderPoints, side, inner, walk, color, opacity, y, thickness) {
  ...
  const recordEnd = (cut, end, next, awaySide, laidWidth) => {
    ...
    if (leg) {
      pavementCornerLaid.set(`${leg.id}:${awaySide}`,
                             { width: laidWidth, y, thickness, color, opacity });
    }
  };
  ...
  const emit = (from, to) => {
    ...
    if (inset > 0.2) {
      const walkCentre = offsetWay(stretch, side * (inner + KERB_LIP_M + inset / 2));
      ...
      addPavementRibbon(offsetWay(stretch, side * (inner + KERB_LIP_M / 2)), KERB_LIP_M,
                        color, opacity, y, thickness, "kerb");
    }
  };
  let start = 0;
  for (let i = 1; i < chosen.length; i += 1) {
    if (chosen[i] !== chosen[start]) {
      emit(start, i - 1);
      start = i;
    }
  }
...
//: A surface car park: asphalt, and bays 2.5 m by 5.0 m painted in rows along its long axis.
...
function addParkingLot(way) {
  ...
  // The bays: rows along the lot's long axis, from its minimum-area rectangle. Each row is a
  // line of stalls PARKING_BAY_L_M deep with an aisle between facing rows, which is how a lot is
  // laid out, and the stalls stop where the polygon does.
  const rect = roofRectangle(way);
  if (!rect) return group;
  // roofRectangle's "width" is the long side and "depth" the short one, at "angle".
  const { x: cx, z: cz, angle, width: length, depth: width } = rect;
  const ux = Math.cos(angle), uz = Math.sin(angle);          // along the long axis
  const vx = -uz, vz = ux;                                   // across it
  const inside = (x, z) => {
    // Even-odd, on the lot's own ring: a stall is only painted where there is lot under it.
    let hit = false;
    for (let i = 0, j = ring.length - 1; i < ring.length; j = i++) {
      const [xi, zi] = ring[i];
      const [xj, zj] = ring[j];
  ...
  const stripes = [];
  const period = PARKING_BAY_L_M * 2 + PARKING_AISLE_M;      // two facing rows and an aisle
  for (let v = -width / 2 + PARKING_BAY_L_M; v <= width / 2 - PARKING_BAY_L_M; v += period) {
    // Two rows back to back across v: stalls from v-L to v, and from v to v+L, when there is
    // room on both; the separating line is shared.
    for (const [v0, v1] of [[v - PARKING_BAY_L_M, v], [v, v + PARKING_BAY_L_M]]) {
      if (v0 < -width / 2 || v1 > width / 2) continue;
      for (let u = -length / 2 + 1.0; u <= length / 2 - 1.0; u += PARKING_BAY_W_M) {
  ...
  for (const record of records) {
    ...
    for (const run of runs) {
      ...
      for (let i = 1; i < run.length; i += 1) {
        ...
        // Into the road from the kerb: the inward normal is the way a bus is parked.
        const ain = BUS_ZONE_WIDTH_M;
        quad(fill,
          a.x, a.z,
          b.x, b.z,
...
window.kerbside = {
  ...
  junctionBoxes,
  kerbVerticesDropped,
  nearestKerbAt, furnitureAnchor, bestFacadeWall, wallPoint, shopBays,
