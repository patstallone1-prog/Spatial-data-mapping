...
function yieldFrame(progress) {
  ...
  // A frame when the browser gives one; a timer when it does not, so that a page building in a
  // background tab -- where requestAnimationFrame never fires -- still finishes building.
...
//: Every fourth cell: an eight-metre grid. The surfaces standing on it are refined to twelve
//: metres, so a finer backdrop under them was a million triangles a frame for nothing.
const TERRAIN_MESH_STRIDE = 4;
...
const TERRAIN_REFINE_MIN_M = 5.0;
...
//: The underlay's top is 17 cm under the pavement's; its chords may leave the ground by less
//: than that, or they stand up through it.
const TERRAIN_UNDER_CHORD_M = 0.08;
...
function refineForTerrain(geometry, maxEdge = TERRAIN_REFINE_M, chord = TERRAIN_CHORD_M) {
  ...
  const cutAt = (a, b, len) => {
    ...
    if (!TERRAIN_FRAME || !(chord > 0) || len <= TERRAIN_REFINE_MIN_M || count > budget) return -1;
    const mid = terrainHeightAt((px(a) + px(b)) / 2, (pz(a) + pz(b)) / 2);
    return Math.abs(mid - (terrainHeightAt(px(a), pz(a)) + terrainHeightAt(px(b), pz(b))) / 2) > chord ? 0.5 : -1;
  };
...
//: What stands on the pavement stands on the pavement as drawn. A tactile pad is a small quad
//: laid at the pavement's height over the ground grid; the pavement under it is a large
//: triangle lifted at its own vertices, and between them it runs on its chord, which on a hill
//: is centimetres off the grid -- eight and a half at Montgomery and Clay. So the pad was under
//: the pavement wherever the chord ran high. No height read from the grid can know that; only
//: the drawn surface can. After the streets are lifted, the pavement's triangles are indexed by
//: cell, and every vertex of a pad and its ramp is set from the pavement's rendered top under
//: it (the highest, where pavements overlap) plus its own few millimetres.
const PAVEMENT_INDEX_CELL_M = 2.0;
const PAVEMENT_SURFACES = new Set(["walk", "walk_narrow"]);
const SETTLE_ABOVE_M = { tactile_warning: 0.018, curb_ramp: 0.016 };
function indexPavementTriangles() {
  const cells = new Map();
  const tris = [];      // flat: x0,y0,z0, x1,y1,z1, x2,y2,z2
  groups.streets.traverse((o) => {
    if (!o.isMesh || !PAVEMENT_SURFACES.has(o.userData.surface) || !o.geometry) return;
    ...
    for (let i = 0; i + 2 < n; i += 3) {
      ...
      tris.push(position.getX(a), position.getY(a), position.getZ(a),
                position.getX(b), position.getY(b), position.getZ(b),
                position.getX(c), position.getY(c), position.getZ(c));
      const x0 = Math.floor(Math.min(tris[id * 9], tris[id * 9 + 3], tris[id * 9 + 6]) / PAVEMENT_INDEX_CELL_M);
      const x1 = Math.floor(Math.max(tris[id * 9], tris[id * 9 + 3], tris[id * 9 + 6]) / PAVEMENT_INDEX_CELL_M);
      const z0 = Math.floor(Math.min(tris[id * 9 + 2], tris[id * 9 + 5], tris[id * 9 + 8]) / PAVEMENT_INDEX_CELL_M);
      const z1 = Math.floor(Math.max(tris[id * 9 + 2], tris[id * 9 + 5], tris[id * 9 + 8]) / PAVEMENT_INDEX_CELL_M);
  ...
  const topAt = (x, z) => {
    const list = cells.get(Math.floor(x / PAVEMENT_INDEX_CELL_M) * 1048576 + Math.floor(z / PAVEMENT_INDEX_CELL_M));
    ...
    for (const id of list) {
      ...
      const y = u * tris[o + 1] + v * tris[o + 4] + w * tris[o + 7];
      if (top === undefined || y > top) top = y;
    }
    return top;
  };
  return { topAt, triangles: tris.length / 9 };
}
async function settleOntoPavement() {
  if (!TERRAIN_FRAME) return 0;
  const { topAt } = indexPavementTriangles();
  await maybeYield("Setting the pads on the pavement");
  let settled = 0;
  const meshes = [];
  groups.streets.traverse((o) => { if (o.isMesh && SETTLE_ABOVE_M[o.userData.surface] !== undefined) meshes.push(o); });
  for (const o of meshes) {
    const above = SETTLE_ABOVE_M[o.userData.surface];
    const position = o.geometry.getAttribute("position");
    for (let i = 0; i < position.count; i += 1) {
      const top = topAt(position.getX(i), position.getZ(i));
      if (top === undefined) continue;
      position.setY(i, top + above);
      settled += 1;
    }
    ...
    if (clipToRoad) {
      ...
      for (let t = 0; t + 2 < count; t += 3) {
        const [ia, ib, ic] = index
          ? [index.getX(t), index.getX(t + 1), index.getX(t + 2)]
          : [t, t + 1, t + 2];
        const cx = (pos.getX(ia) + pos.getX(ib) + pos.getX(ic)) / 3;
        const cy = (pos.getY(ia) + pos.getY(ib) + pos.getY(ic)) / 3;
        if (!insideCarriageway(cx, -cy, 0.15)) continue;
        // The centre alone is not enough. A park is a handful of very large triangles, and one
        // of those can have its centre over a road that clips a corner of the park while the
        // park itself is nowhere near it -- dropping that triangle takes out the park. Two
        // corners on the carriageway as well means the triangle really is lying in the street.
        const corners = (insideCarriageway(pos.getX(ia), -pos.getY(ia), 0.15) ? 1 : 0)
          + (insideCarriageway(pos.getX(ib), -pos.getY(ib), 0.15) ? 1 : 0)
          + (insideCarriageway(pos.getX(ic), -pos.getY(ic), 0.15) ? 1 : 0);
        if (corners >= 2) drop.add(t);
      }
      for (let t = 0; t + 2 < count; t += 3) {
        if (drop.has(t)) continue;
        if (index) indices.push(base + index.getX(t), base + index.getX(t + 1),
                                base + index.getX(t + 2));
        else indices.push(base + t, base + t + 1, base + t + 2);
      }
    } else if (index) {
