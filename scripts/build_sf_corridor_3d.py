...
#: A mouth this close to a street crossing over the tunnel's line is under that street's
#: junction -- Bush Street stands on the Stockton Tunnel's south portal -- and the flat model
#: cannot draw a junction eight metres up. The mouth is moved to the far side of the junction
#: and the piece before it is drawn as the open street it is at grade.
TUNNEL_MOUTH_PUSH_M = 24.0
TUNNEL_MOUTH_PAST_JUNCTION_M = 3.0
...
def _crossing_street_along(points: list[list[float]], s_from: float, s_to: float,
                           streets: list[dict[str, Any]], own: dict[str, Any]) -> float | None:
    """How far along the way the last edge of a crossing street's carriageway lies, between
    s_from and s_to; None when no street crosses there."""
    def m(p: list[float], o: list[float]) -> tuple[float, float]:
        return ((p[0] - o[0]) * 88_000.0, (p[1] - o[1]) * 111_320.0)
    origin = points[0]
    line = [m(p, origin) for p in points]
    far = None
    for street in streets:
        if street is own or street.get("tunnel") or street.get("kind") != "street":
            continue
        pts = street.get("points") or []
        if len(pts) < 2:
            continue
        half = (street.get("road_m") or 10.0) / 2
        acc = 0.0
        for a, b in zip(line[:-1], line[1:], strict=True):
            seg = math.hypot(b[0] - a[0], b[1] - a[1])
            if seg <= 0:
                continue
            for c, d in zip(pts[:-1], pts[1:], strict=True):
                cx, cy = m(c, origin)
                dx, dy = m(d, origin)
                # Segment intersection of the way piece a-b with the street piece c-d.
                den = (b[0] - a[0]) * (dy - cy) - (b[1] - a[1]) * (dx - cx)
                if abs(den) < 1e-9:
                    continue
                t = ((cx - a[0]) * (dy - cy) - (cy - a[1]) * (dx - cx)) / den
                u = ((cx - a[0]) * (b[1] - a[1]) - (cy - a[1]) * (b[0] - a[0])) / den
                if not (0 <= t <= 1 and 0 <= u <= 1):
                    continue
                # The street's far kerb, along the way: its half width over the sine of the
                # angle between the two.
                cross = abs(((b[0] - a[0]) * (dy - cy) - (b[1] - a[1]) * (dx - cx))
                            / (seg * (math.hypot(dx - cx, dy - cy) or 1)))
                if cross < 0.5:
                    continue
                s = acc + seg * t + half / cross
                if s_from <= acc + seg * t <= s_to and (far is None or s > far):
                    far = s
            acc += seg
    return far
...
def split_tunnel_approaches(ways: list[dict[str, Any]]) -> dict[str, int]:
    ...
    A mouth within TUNNEL_MOUTH_PUSH_M of a street crossing the line is under that street's
    junction (Bush Street stands on the Stockton Tunnel's south portal). The flat model draws
    the junction at grade, so the mouth is moved past it and the tunnel starts on its far
    side; the move is recorded in ``tunnel_mouth_pushed_m``.
    """
    counts: Counter[str] = Counter()
    ...
    for way in ways:
        ...
        pushed = {}
        far = _crossing_street_along(points, s0, s0 + TUNNEL_MOUTH_PUSH_M, streets, way)
        if far is not None and far > s0:
            pushed["start"] = round(far + TUNNEL_MOUTH_PAST_JUNCTION_M - s0, 1)
            s0 = far + TUNNEL_MOUTH_PAST_JUNCTION_M
        reversed_points = points[::-1]
        far_end = _crossing_street_along(reversed_points, total - s1,
                                         total - s1 + TUNNEL_MOUTH_PUSH_M, streets, way)
        if far_end is not None and far_end > total - s1:
            pushed["end"] = round(far_end + TUNNEL_MOUTH_PAST_JUNCTION_M - (total - s1), 1)
            s1 = total - (far_end + TUNNEL_MOUTH_PAST_JUNCTION_M)
        ...
        for a, b, label in ((0.0, s0, "start"), (s1, total, "end")):
            ...
            approach = {k: v for k, v in way.items()
                        if k not in ("tunnel", "tunnel_kind", "tunnel_length_m", "tunnel_portal",
                                     "tunnel_mouths", "tunnel_cover", "layer")}
        ...
        if pushed:
            way["tunnel_mouth_pushed_m"] = pushed
            counts["mouth moved past a junction"] += len(pushed)
        if way.get("tunnel_cover"):
            shift = pushed.get("start", 0.0)
...
root.add(ground);
//: The planes at the bottom of the world, so the tunnels can be cut out of them: a bore sunk
//: below the flat ground (TUNNEL_SINK_M) would otherwise have the land plane and the bay
//: running straight through it at head height.
const GROUND_PLANES = [ground];
...
function boxPart(group, x, y, z, w, h, d, color, roughness = 0.82) {
  const mesh = makeBox(x, y, z, w, h, d, color, roughness);
...
function slab(group, x, y, z, w, h, d, material, rotY = 0, tilt = 0, tiltAxis = "x") {
  ...
  ROOF_PARTS.get(key).boxes.push([x, y + h / 2, z, w, h, d, rotY, tilt, tiltAxis === "x" ? 0 : 1]);
  return null;
}
...
//: The model is flat: there is no hill for the bore to go through, so the bore goes down
//: instead. From each mouth its floor descends at TUNNEL_GRADE to TUNNEL_SINK_M below the
//: ground and runs there to the far mouth, under the streets and houses the flat model draws
//: at grade on Russian Hill. The hill drawn at the mouth is as long as the descent needs
//: (TUNNEL_RAMP_M) unless a street crosses the line sooner; past the hill the roof is held
//: below the surface (TUNNEL_CEILING_CAP_M) -- a flat cut-and-cover roof under the junction
//: -- until the arch fits. A walker goes in one mouth and out the other.
...
function tunnelStations(centre, moundStart, moundEnd) {
  const local = centre.map((p) => { const [x, y] = xy(p[0], p[1]); return [x, -y]; });
  const total = wayLength(centre);
  const stations = [];
  const floorAt = (s) => -Math.min(TUNNEL_SINK_M, TUNNEL_GRADE * s, TUNNEL_GRADE * (total - s));
  let acc = 0;
  for (let i = 1; i < local.length; i += 1) {
    ...
    for (let k = i === 1 ? 0 : 1; k <= steps; k += 1) {
      const t = k / steps;
      const s = acc + seg * t;
      const inHill = s <= moundStart || s >= total - moundEnd;
      stations.push({
        s, x: ax + ux * seg * t, z: az + uz * seg * t, ux, uz, rx: -uz, rz: ux,
        floor: TUNNEL_FLOOR_M + floorAt(s),
        cap: inHill ? Infinity : Math.max(TUNNEL_WALK_H_M + 0.4, TUNNEL_CEILING_CAP_M - (TUNNEL_FLOOR_M + floorAt(s))),
      });
    }
    acc += seg;
  }
...
function tunnelCoverAt(x, z) {
  ...
  for (const entry of tunnelCoverIndex) {
    const { points, halfWidth, cover, zones, bbox } = entry;
    if (bbox && (x < bbox[0] || x > bbox[2] || z < bbox[1] || z > bbox[3])) continue;
    let along = 0;
    for (let i = 1; i < points.length; i += 1) {
...
//: Each bore's footprint in world x/z, the rings the planes at the bottom of the world are cut
//: along, and the hill at each mouth as an oriented box the surface network is not drawn in.
const tunnelVoids = [];
...
function walkerGroundAt(x, z) {
  ...
  return tunnelCoverIndex.length ? tunnelCoverAt(x, z) : 0;
}
const TUNNEL_HILL_BOXES = [];
const TUNNEL_HILL_BOX_MAX = 8;
const tunnelCullUniforms = {
  uTunnelBoxA: { value: Array.from({ length: TUNNEL_HILL_BOX_MAX }, () => new THREE.Vector4()) },
  uTunnelBoxB: { value: Array.from({ length: TUNNEL_HILL_BOX_MAX }, () => new THREE.Vector4()) },
  uTunnelBoxCount: { value: 0 },
};

function registerTunnelHillBox(x, z, ux, uz, length, halfWidth, top) {
  if (TUNNEL_HILL_BOXES.length >= TUNNEL_HILL_BOX_MAX) return;
  TUNNEL_HILL_BOXES.push({ x, z, ux, uz, length, halfWidth, top });
  const i = TUNNEL_HILL_BOXES.length - 1;
  tunnelCullUniforms.uTunnelBoxA.value[i].set(x, z, ux, uz);
  tunnelCullUniforms.uTunnelBoxB.value[i].set(length, halfWidth, top, 0);
  tunnelCullUniforms.uTunnelBoxCount.value = TUNNEL_HILL_BOXES.length;
}

//: Whether (x, z, y) is inside one of the hills drawn at a tunnel mouth: what the shader
//: patch below asks of every fragment, for the audit and the probe to ask of a point.
function underTunnelHill(x, z, y = 0) {
  for (const b of TUNNEL_HILL_BOXES) {
    const dx = x - b.x, dz = z - b.z;
    const along = dx * b.ux + dz * b.uz;
    const across = dx * -b.uz + dz * b.ux;
    if (along >= 0 && along <= b.length && Math.abs(across) <= b.halfWidth && y < b.top) return true;
  }
  return false;
}

//: The surface network is not drawn inside the hill at a tunnel mouth. The pavements, ground
//: cover and paint along the tunnel's line are laid at ground level like everything else in
//: the flat model, and the bore descends through them: from inside the mouth they stood in
//: the air across the roadway. Every ground material discards its fragments inside the hill
//: boxes; the hill, the headwall, the bore's own lining and what stands on the hill are not
//: patched, so from outside nothing changes.
function cullUnderTunnelHills(material) {
  if (!material || material.userData.tunnelCulled) return material;
  material.userData.tunnelCulled = true;
  const previous = material.onBeforeCompile;
  material.onBeforeCompile = (shader, renderer) => {
    if (previous) previous(shader, renderer);
    shader.uniforms.uTunnelBoxA = tunnelCullUniforms.uTunnelBoxA;
    shader.uniforms.uTunnelBoxB = tunnelCullUniforms.uTunnelBoxB;
    shader.uniforms.uTunnelBoxCount = tunnelCullUniforms.uTunnelBoxCount;
    ...
    shader.fragmentShader = shader.fragmentShader
      .replace("#include <common>",
               `#include <common>
varying vec3 vTunnelWorld;
uniform vec4 uTunnelBoxA[${TUNNEL_HILL_BOX_MAX}];
uniform vec4 uTunnelBoxB[${TUNNEL_HILL_BOX_MAX}];
uniform int uTunnelBoxCount;`)
      .replace("#include <clipping_planes_fragment>",
               `#include <clipping_planes_fragment>
for (int ti = 0; ti < ${TUNNEL_HILL_BOX_MAX}; ti++) {
  if (ti >= uTunnelBoxCount) break;
  vec4 ta = uTunnelBoxA[ti];
  vec4 tb = uTunnelBoxB[ti];
  vec2 td = vTunnelWorld.xz - ta.xy;
  float tAlong = dot(td, ta.zw);
  float tAcross = td.x * -ta.w + td.y * ta.z;
  if (tAlong >= 0.0 && tAlong <= tb.x && abs(tAcross) <= tb.y && vTunnelWorld.y < tb.z) discard;
}`);
  };
  material.customProgramCacheKey = () => "tunnel-hill-cull";
  material.needsUpdate = true;
  return material;
}

//: Every ground-level material under a group: the streets, their paint and kerbs, the ground
//: cover. Not the tunnels' own meshes.
function cullGroupUnderTunnelHills(group) {
  group.traverse((o) => {
    if (!o.isMesh || !o.material) return;
    if (String((o.userData || {}).surface || "").startsWith("tunnel")) return;
    for (const m of Array.isArray(o.material) ? o.material : [o.material]) cullUnderTunnelHills(m);
  });
}

//: The planes at the bottom of the world with the bores cut out of them. A plane is a
//: rectangle or a disc in its own x/y, laid flat by a quarter turn about x, so a world (x, z)
//: is (x - px, -(z - pz)) on it.
function cutTunnelVoidsFromPlanes() {
  if (!tunnelVoids.length) return 0;
  let cut = 0;
  for (const plane of GROUND_PLANES) {
    ...
    for (const ring of tunnelVoids) {
      const hole = new THREE.Path();
      ring.forEach(([x, z], i) => {
        const lx = x - px, ly = -(z - pz);
        if (i === 0) hole.moveTo(lx, ly); else hole.lineTo(lx, ly);
      });
      shape.holes.push(hole);
    }
    const old = plane.geometry;
    plane.geometry = new THREE.ShapeGeometry(shape, params.radius ? 96 : 1);
    old.dispose();
    cut += 1;
  }
  return cut;
...
function addTunnel(way, renderPoints, roadWidth, roadTop) {
  ...
  // bores with a pier between them -- Broadway's are two tubes with a wall between, each
  // carrying one direction, which is what its portal shows.
  ...
  // The hill at each mouth: as long as the descent needs, or as far as the first street
  // crossing the line, whichever comes first.
  const mounds = {};
  for (const { label, mouth, inward } of ends) {
    mounds[label] = Math.min(tunnelClearRun(way, mouth, inward), TUNNEL_RAMP_M, total / 2);
  }
  // The bore's footprint, for the planes at the bottom of the world to be cut along.
  {
    const reach = width / 2 + TUNNEL_SHELL_M + 1.0;
    const left = offsetWay(covered, reach).map((p) => { const [x, y] = xy(p[0], p[1]); return [x, -y]; });
    const right = offsetWay(covered, -reach).map((p) => { const [x, y] = xy(p[0], p[1]); return [x, -y]; });
    if (left.length > 1 && right.length > 1) tunnelVoids.push(left.concat(right.reverse()));
  }
  // The bores, swept the whole way between the mouths, going down under the flat ground.
  const oneway = Boolean(way.oneway || way.osm_oneway);
  for (const off of offsets) {
    // The centreline of this bore, offset from the way's: the headwall's holes are cut at
    // +off to the left of travel, and offsetWay's positive side is the left too.
    const line = offsetWay(covered, off);
    const stations = tunnelStations(line, mounds.start || 0, mounds.end || 0);
    ...
    // The walkways, one along each wall.
    for (const side of [-1, 1]) {
    ...
    // The paint: a two-way bore has its double yellow, a one-way bore a broken white lane line.
  ...
  for (const { label, pts, inward, mouth } of ends) {
    const here = pts[0];
    const approach = approaches.length > 0;
    const stub = mounds[label];
    ...
    portal.position.set(here.x, roadTop, here.z);
    ...
    // showing through the arch, and what the buildings up there stand on. The throat behind
    // the headwall has the arches cut through it; the body of the hill behind that is a shell
    // -- top, sides and back -- with the bores descending inside it, so from the portal the
    // eye goes straight into the lining and from the surface the hill is solid.
    const cover = Math.max(H, measured[label] || 0);
    registerTunnelHillBox(mouth.x, mouth.z, inward.x, inward.z, stub + TUNNEL_PORTAL_DEPTH_M, W, cover);
    const throatDepth = Math.min(stub, TUNNEL_PORTAL_DEPTH_M + 1.0);
    // The face reaches below the arches' feet, so each hole lies wholly inside it: a hole
    // that crosses the outline is not a hole to the triangulator, and the arch filled in.
    const foot = -TUNNEL_SHELL_M;
    const hillFace = new THREE.Shape();
    hillFace.moveTo(-W, foot); hillFace.lineTo(W, foot); hillFace.lineTo(W, cover); hillFace.lineTo(-W, cover); hillFace.lineTo(-W, foot);
    for (const off of offsets) {
      const arch = tunnelArchShape(half, true).getPoints(28).map((q) => new THREE.Vector2(q.x + off, q.y));
      hillFace.holes.push(new THREE.Path(arch));
    }
    const throat = new THREE.Mesh(new THREE.ExtrudeGeometry(hillFace, { depth: throatDepth, bevelEnabled: false }), hill);
    throat.position.set(here.x + inward.x * TUNNEL_PORTAL_DEPTH_M, TUNNEL_FLOOR_M - 0.05, here.z + inward.z * TUNNEL_PORTAL_DEPTH_M);
    throat.rotation.y = yaw;
    throat.userData.surface = "tunnel_hill";
    throat.userData.coverM = +cover.toFixed(2);
    throat.userData.measured = Boolean(measured[label]);
    group.add(throat);
    const bodyDepth = stub - throatDepth;
    if (bodyDepth > 0.5) {
      // Top, two sides, back: no front (the throat closes it) and no bottom.
      const y0 = TUNNEL_FLOOR_M - 0.05, y1 = TUNNEL_FLOOR_M - 0.05 + cover;
      const z0 = 0, z1 = bodyDepth;
      const v = [
        -W, y1, z0,  W, y1, z0,  W, y1, z1,  -W, y1, z1,          // top
        -W, y0, z0,  -W, y1, z0,  -W, y1, z1,  -W, y0, z1,        // left side
        W, y0, z0,  W, y1, z0,  W, y1, z1,  W, y0, z1,            // right side
        -W, y0, z1,  W, y0, z1,  W, y1, z1,  -W, y1, z1,          // back
      ];
      const idx = [];
      for (let q = 0; q < 4; q += 1) { const b = q * 4; idx.push(b, b + 1, b + 2, b, b + 2, b + 3); }
      const geometry = new THREE.BufferGeometry();
      geometry.setAttribute("position", new THREE.Float32BufferAttribute(v, 3));
      geometry.setIndex(idx);
      geometry.computeVertexNormals();
      const body = new THREE.Mesh(geometry, hill);
      body.position.set(here.x + inward.x * (TUNNEL_PORTAL_DEPTH_M + throatDepth), 0,
                        here.z + inward.z * (TUNNEL_PORTAL_DEPTH_M + throatDepth));
      body.rotation.y = yaw;
      body.userData.surface = "tunnel_hill";
      body.userData.coverM = +cover.toFixed(2);
      group.add(body);
    }
    halves.push({ label, width, bores, stub, cover, sinkM: TUNNEL_SINK_M, gradePct: TUNNEL_GRADE * 100 });
  }
...
for (const way of DATA.ways) {
  ...
  const covered = tunnelAxis(way, tunnelCoveredPoints(way, densifyWay(way.points)));
  ...
  for (const [label, a, b] of [["start", local[0], local[1]],
    ...
    const stub = Math.min(tunnelClearRun(way, { x: a[0], z: a[1] }, { x: -dx / len, z: -dz / len }), TUNNEL_RAMP_M, total / 2);
    const cover = Math.max(H, (way.tunnel_portal || {})[label] || 0);
    zones.push(label === "start" ? [TUNNEL_PORTAL_DEPTH_M, stub + TUNNEL_PORTAL_DEPTH_M, cover]
                                 : [total - stub - TUNNEL_PORTAL_DEPTH_M, total - TUNNEL_PORTAL_DEPTH_M, cover]);
  }
  const reach = width / 2 + TUNNEL_HILL_HALF_EXTRA_M;
  const bbox = [Math.min(...local.map((q) => q[0])) - reach, Math.min(...local.map((q) => q[1])) - reach,
                Math.max(...local.map((q) => q[0])) + reach, Math.max(...local.map((q) => q[1])) + reach];
  tunnelCoverIndex.push({ points: local, halfWidth: width / 2, cover: way.tunnel_cover, zones, bbox });
}
for (const way of DATA.ways) {
...
function addGround(mesh, what) {
  ...
  cullUnderTunnelHills(mesh.material);
  groups.ground.add(mesh);
  return mesh;
}
...
(async () => {
  ...
  buildCourts(ground);
  cullGroupUnderTunnelHills(groups.ground);
...
// and nothing at ground level is drawn inside the hill at a mouth.
cutTunnelVoidsFromPlanes();
cullGroupUnderTunnelHills(groups.streets);
...
function goTo(landing, { travel }) {
  if (!landing) return;
  if (travel) {
    avatar.position.set(landing.x, AVATAR_STAND_Y, landing.z);
    state.dist = Math.min(state.dist, ARRIVAL_DIST);
  }
  state.target.set(landing.x, travel ? AVATAR_STAND_Y : 0, landing.z);
  placeCamera();
}
...
window.kerbside = {
  ...
  tunnelFloorAt, walkerGroundAt, tunnelBores, underTunnelHill, TUNNEL_HILL_BOXES,
  ...
  twinInnerEdgeAt, halfWidthAt, filletBends, bendsFilleted, tunnelCoverAt, tunnelCoverIndex, tunnelCoveredPoints, tunnelApproachWidth,
  ...
  shoot({ lon, lat, from = 180, tilt = 52, heading = 30, w = 720, h = 405, fov = 45,
          quality = 0.72, targetY = 0 } = {}) {
    const [x, y] = xy(lon, lat);
    const target = new THREE.Vector3(x, targetY, -y);
...
PAGE_FIELDS = {
    ...
    "tunnel_portal", "tunnel_mouths", "tunnel_cover", "tunnel_mouth_pushed_m",
    "tunnel_approach", "parking_sides", "xs", "xs_junction",
    "capacity",
    "surface",
    "material",
    "wall",
