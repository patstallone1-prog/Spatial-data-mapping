...
function addBikeLaneMarkings(way, renderPoints, roadWidth, roadTop) {
  ...
  for (const side of sides) {
    ...
    if (shiftedFill) groups.streets.add(shiftedFill);
    ...
    if (outer) groups.streets.add(outer);
    if (inner) groups.streets.add(inner);
  }
}
...
function roofTextureFor(tint, seed, archetype) {
  const colour = new THREE.Color(tint).multiplyScalar(0.80);
  const hsl = {};
  colour.getHSL(hsl);
  if (hsl.l < 0.22) colour.setHSL(hsl.h, hsl.s, 0.22);
  const bucket = colour.getHex();
  const key = `${archetype || "generic"}-${bucket}-${Math.floor(random(seed + 13) * 4)}`;
  if (!ROOF_CACHE.has(key)) ROOF_CACHE.set(key, roofTexture(seed, bucket, archetype));
...
function rotatedBoxPart(group, x, y, z, w, h, d, color, roughness = 0.82, rotationY = 0) {
  const mesh = boxPart(group, x, y, z, w, h, d, color, roughness);
...
function buildingMesh(feature) {
  ...
  const roof = new THREE.MeshStandardMaterial({
    map: roofTextureFor(tint, seed, feature.archetype),
  ...
  if (kind === "outbuilding") {
    ...
    walls.map = outbuildingTexture(seed);
    walls.envMapIntensity = 0.25;
    roof.map = outbuildingRoofTexture(seed);
  } else {
...
groups.streets.add(ribbon(surfacePoints, widthMeters, color, opacity, surfaceY, surfaceThickness, surfaceKind));
}
    ...
    if (total >= 2) {
      ...
      if (laneWidth >= 2.6) {
        for (let i = 1; i < total; i += 1) {
          ...
          if (divider) groups.streets.add(divider);
        }
      }
    }
    if (opposing) {
      ...
      for (const side of [1, -1]) {
        const yellow = paintedLine(offsetWay(renderPoints, side * apart), MARK_W, 0xd8a92e,
                                   roadTop + 0.02);
        if (yellow) groups.streets.add(yellow);
      }
    }
  }
...
window.kerbside = {
  scene, camera, renderer, groups, DATA, xy, insideCarriageway,
