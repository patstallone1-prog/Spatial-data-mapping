...
def annotate_official(ways: list[dict[str, Any]], bbox: dict) -> dict[str, Any]:
    ...
    counts["plain crossing"] = len(crossings) - len(claimed) - siblings
...
function walkSidesToDraw(way, renderPoints, inner) {
  ...
  const drawn = [];
  if (isUnmarkedService(way)) return drawn;
...
function kerbsideTrims(renderPoints, back, side = 0) {
  ...
  const cornerCut = (x, y, dx, dy, awaySide) => {
    if (!awaySide) return undefined;
    const leg = cornerLegAt(x, y, dx, dy);
    const corner = leg && pavementCornerCuts.get(`${leg.id}:${awaySide}`);
    return corner && corner.valid ? corner.cut : undefined;
  };
  const start = cornerCut(sx, sy, s2x - sx, s2y - sy, side);
  // Leaving the far end the way runs backwards, so the pavement on its left is on the right.
  const end = cornerCut(ex, ey, e2x - ex, e2y - ey, -side);
  return [
    start !== undefined ? start : (streetCarriesOn(null, sx, -sy, startBearing) ? 0 : back),
    end !== undefined ? end : (streetCarriesOn(null, ex, -ey, endBearing) ? 0 : back),
  ];
}
...
function cornerLegAt(x, y, dx, dy) {
  // The indexed leg leaving this point in this direction, if any.
...
function cornerLegsAt(leg) {
  const cx = Math.floor(leg.x / CORNER_CELL_M);
  const cy = Math.floor(leg.y / CORNER_CELL_M);
  const out = [];
  for (let ix = cx - 1; ix <= cx + 1; ix += 1) {
    for (let iy = cy - 1; iy <= cy + 1; iy += 1) {
      const bucket = cornerLegGrid.get(`${ix}:${iy}`);
      ...
      for (const other of bucket) {
        ...
        if (Math.hypot(other.x - leg.x, other.y - leg.y) <= CORNER_JOIN_M) out.push(other);
      }
    }
  }
  return out;
...
function indexPavementCorners(ways) {
  ...
  for (const way of ways) {
    ...
    for (const [end, next] of [[0, 1], [n - 1, n - 2]]) {
      ...
      if (span < 1e-6) continue;
      const leg = {
        id: cornerLegs.length, x: ex, y: ey, dx: (nx2 - ex) / span, dy: (ny2 - ey) / span,
      ...
      cornerLegs.push(leg);
      const key = `${Math.floor(ex / CORNER_CELL_M)}:${Math.floor(ey / CORNER_CELL_M)}`;
...
function addKerbsidePavement(renderPoints, side, inner, walk, color, opacity, y, thickness) {
  ...
  const emit = (from, to) => {
    ...
    // The kerb first: four inches of the top and the whole face, as its own tile along the
    // street edge. It is laid with the pavement rather than over it -- the same concrete, the
    // same height, the same merge -- and it is the thing the curb policy paints. Four inches is
    // the default where nothing was measured; SFMTA paints the face and the top edge of the kerb.
    ...
    if (inset > 0.2) {
      laid += addPavementRibbon(offsetWay(stretch, side * (inner + KERB_LIP_M + inset / 2)), inset,
                                color, opacity, y, thickness,
                                inset <= NARROW_WALK_M ? "walk_narrow" : "walk");
    }
  };
  let start = 0;
