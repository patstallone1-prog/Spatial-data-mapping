...
function trimWalkwayForCorners(points, width) {
  // Explicit sidewalk/path ways often run right into the mapped junction node. The road at the
  // corner is already drawn there, and letting concrete ribbons meet over that same point is
  // what creates the messy checkerboard overlap. Pulling only the walkable ribbons back leaves
  // crossings to bridge the gap, including the synthetic alley-mouth crossings below. Do not
  // pull back every OSM way end, though: ways are also split for tagging and editing reasons,
  // and treating those invisible joins as intersections cuts an otherwise continuous pavement
  // into isolated chunks.
  if (!points || points.length < 2) return points || [];
  const touchesJunction = (point) => {
    const [x, y] = xy(point[0], point[1]);
    return insideCrossingArea(x, -y, Math.max(0.12, width / 2))
      || insideCarriageway(x, -y, 0.0)
      || pavementSurroundedByStreet(x, -y, width);
  };
  const cut = intersectionWalkwayCutback(width);
  return trimWayEnds(points,
                     touchesJunction(points[0]) ? cut : 0,
                     touchesJunction(points[points.length - 1]) ? cut : 0);
}
...
function crossingRoadSpanPoints(points) {
  // A crosswalk is the local normal of the street it crosses, from one kerb to the other. OSM
  // crossing ways often include doglegs and sidewalk connector nodes, so their endpoints are a
  // useful direction hint but are not kerb geometry. Find the nearby carriageway segment most
  // nearly perpendicular to that hint and use its own bearing and half-width. This works on the
  // rotated San Francisco grid and on diagonal streets; snapping to global x/z axes does not.
...
function addCrossingAreaSegment(points, width) {
  const rect = crossingRectanglePoints(points);
  if (!rect || rect.length < 2) return;
  const [ax, ay] = xy(rect[0][0], rect[0][1]);
  const [bx, by] = xy(rect[1][0], rect[1][1]);
  const az = -ay;
  const bz = -by;
  const half = Math.max(1.0, (width || 3.7) / 2);
  const x0 = Math.floor((Math.min(ax, bx) - half) / CROSSING_AREA_CELL_M);
  const x1 = Math.floor((Math.max(ax, bx) + half) / CROSSING_AREA_CELL_M);
  const z0 = Math.floor((Math.min(az, bz) - half) / CROSSING_AREA_CELL_M);
  const z1 = Math.floor((Math.max(az, bz) + half) / CROSSING_AREA_CELL_M);
  for (let ix = x0; ix <= x1; ix += 1) {
    for (let iz = z0; iz <= z1; iz += 1) {
      const key = `${ix}:${iz}`;
      let bucket = crossingAreaGrid.get(key);
      if (!bucket) crossingAreaGrid.set(key, bucket = []);
      bucket.push([ax, az, bx, bz, half]);
    }
  }
}

function insideCrossingArea(x, z, slack = 0.08) {
  const cx = Math.floor(x / CROSSING_AREA_CELL_M);
  const cz = Math.floor(z / CROSSING_AREA_CELL_M);
  for (let dx = -1; dx <= 1; dx += 1) {
    for (let dz = -1; dz <= 1; dz += 1) {
      const bucket = crossingAreaGrid.get(`${cx + dx}:${cz + dz}`);
      if (!bucket) continue;
      for (const [ax, az, bx, bz, half] of bucket) {
        const limit = Math.max(0.1, half + slack);
...
function addIntersectionRoadPads(ways, intersections, roadTop) {
  // Street ribbons are good for blocks and bends, but at a large junction each street owns only
  // its strip. The shared asphalt fan between those strips can fall back to the ground plane,
  // which reads as random black/grey patches through turns. A low asphalt cap at real street
  // junction nodes fills that shared road surface while staying under all crosswalk, bike-lane
  // paint, tactile pads, medians and sidewalks.
  const nodes = new Map();
  const widthByName = new Map();
  const touch = (point, names, ids, bearings, half) => {
    const key = `${point[0].toFixed(6)}:${point[1].toFixed(6)}`;
    let node = nodes.get(key);
    if (!node) nodes.set(key, node = { point, names: new Set(), ids: new Set(), bearings: [], half: 0 });
    for (const name of names) node.names.add(name || "");
    for (const id of ids) node.ids.add(id);
    for (const bearing of bearings) node.bearings.push(bearing);
    node.half = Math.max(node.half, half);
  };
  const add = (point, way, bearing, half) => {
    touch(point, [way.name || ""], [way.osm_id || `${point[0]}:${point[1]}:${bearing.toFixed(3)}`],
          [bearing], half);
  };
  for (const way of ways) {
    if (way.kind !== "street" || !way.points || way.points.length < 2) continue;
    const half = renderedRoadWidth(way) / 2;
    if (way.name) widthByName.set(way.name, Math.max(widthByName.get(way.name) || 0, half));
    for (let i = 0; i < way.points.length; i += 1) {
      if (i > 0) {
        ...
        add(way.points[i], way, Math.atan2(-by + ay, bx - ax), half);
      }
      if (i < way.points.length - 1) {
        const [ax, ay] = xy(way.points[i][0], way.points[i][1]);
        const [bx, by] = xy(way.points[i + 1][0], way.points[i + 1][1]);
        add(way.points[i], way, Math.atan2(-by + ay, bx - ax), half);
      }
    }
  }
  for (const crossing of intersections || []) {
    if (crossing.lon === undefined || crossing.lat === undefined) continue;
    const a = crossing.a || "";
    const b = crossing.b || "";
    const half = Math.max(6.0, widthByName.get(a) || 0, widthByName.get(b) || 0);
    touch([crossing.lon, crossing.lat], [a, b], [`intersection:${a}`, `intersection:${b}`],
          [], half);
  }
  ...
  for (const node of nodes.values()) {
    const named = [...node.names].filter(Boolean);
    let turn = 0;
    for (let i = 0; i < node.bearings.length; i += 1) {
      for (let j = i + 1; j < node.bearings.length; j += 1) {
        turn = Math.max(turn, angleBetween(node.bearings[i], node.bearings[j]));
      }
    }
    if (node.ids.size < 2 || (named.length < 2 && turn < Math.PI / 5)) continue;
    const [x, y] = xy(node.point[0], node.point[1]);
    const radius = Math.max(10.0, Math.min(34.0, node.half * 2.2 + 8.0));
    // This disk only hides cracks between rendered road ribbons. It is deliberately absent from
    // carriagewayGrid: a circular paint backstop is not surveyed kerb geometry, and treating it
    // as one stretches crosswalks to the circle and erases the real sidewalks around it.
    const geometry = new THREE.CircleGeometry(radius, 36);
    geometry.rotateX(-Math.PI / 2);
    const uv = [];
    const pos = geometry.attributes.position;
    for (let i = 0; i < pos.count; i += 1) {
      uv.push(pos.getX(i) / ROAD_SLAB_M, pos.getZ(i) / ROAD_SLAB_M);
    }
    geometry.setAttribute("uv", new THREE.Float32BufferAttribute(uv, 2));
    const mesh = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({
      map: ROAD, color: 0xffffff, roughness: 0.86, metalness: 0.02,
      side: THREE.DoubleSide, polygonOffset: true, polygonOffsetFactor: 4,
      polygonOffsetUnits: 8,
    }));
    mesh.position.set(x, roadTop + 0.001, -y);
    mesh.userData.surface = "road";
    addMerged("road:junction", mesh, "road");
  }
}
...
function addLaneTransitionMarkings(way, renderPoints, roadWidth, roadTop) {
  ...
  for (const [other, atStart] of [[startOther, true], [endOther, false]]) {
    ...
    for (const mark of herePaints) {
      ...
      const taper = paintedLine(points, MARK_W, mark.color, roadTop + mark.y);
...
function addSidewalkCrossingReplacements(points, y) {
  ...
  for (const span of sidewalkCrossingReplacementSpans(points, crossingWidth)) {
    ...
    addCrossingAreaSegment(span, crossingWidth);
...
registerLazyLayer("official", async () => {
  let official;
  try {
...
for (const way of DATA.ways) {
  ...
  addCrossingAreaSegment(way.points, way.crossing_m || 3.7);
}
addIntersectionRoadPads(DATA.ways, DATA.intersections, ROAD_TOP_M);
...
const surfaceKind = isCrossing ? ((way.continental || way.alley_mouth) ? "crossing" : "crossing_edges")
  ...
  if (isSidewalk || isPath) {
    ...
    addPavementRibbon(surfacePoints, widthMeters, color, opacity, surfaceY, surfaceThickness, surfaceKind);
  } else {
    addMerged(`ribbon:${surfaceKind}:${(Math.round(surfaceThickness / 0.05) * 0.05).toFixed(2)}`,
              ribbon(surfacePoints, widthMeters, color, opacity, surfaceY, surfaceThickness, surfaceKind),
              surfaceKind);
    if (isCrossing) {
...
function addOfficialStopSigns(records) {
  ...
  const dummy = new THREE.Object3D();
  anchors.forEach((row, i) => {
    const yaw = furnitureBearing(row.item, row.index);
...
function addOfficialCurbZoneBands(records) {
  ...
  const materialFor = (color) => {
    if (!materialByColor.has(color)) {
      materialByColor.set(color, new THREE.MeshStandardMaterial({
        color,
        roughness: 0.86,
        metalness: 0.0,
        side: THREE.DoubleSide,
  ...
  for (const record of records) {
    ...
    for (const coords of record.lines) {
      ...
      for (let i = 1; i < coords.length; i += 1) {
        ...
        const px = -dz / len * bandWidth / 2;
        const pz = dx / len * bandWidth / 2;
