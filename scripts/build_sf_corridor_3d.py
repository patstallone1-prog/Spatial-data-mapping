...
function paintedLine(points, width, color, y, { dash = false } = {}) {
...
function addBikeLaneMarkings(way, renderPoints, roadWidth, roadTop) {
  ...
  const hasTurn = Boolean(way.turn || way.turn_fwd || way.turn_back);
  const linePoints = trimWay(renderPoints, hasTurn ? 12.0 : 3.5);
  const lanePoints = trimWay(renderPoints, hasTurn ? 9.0 : 2.5);
  if (!linePoints || linePoints.length < 2 || !lanePoints || lanePoints.length < 2) return;
  for (const side of sides) {
    const laneCentre = side * Math.max(0.8, roadWidth / 2 - BIKE_LANE_M / 2 - 0.18);
    const shiftedFill = ribbon(offsetWay(lanePoints, laneCentre), BIKE_LANE_M, 0xffffff, 1.0,
      roadTop + 0.010, 0.012, "bike");
    if (shiftedFill) addMerged("bike:fill", shiftedFill, "bike");
    const outerOffset = side * (roadWidth / 2 - 0.12);
    const innerOffset = side * Math.max(0.45, roadWidth / 2 - BIKE_LANE_M);
    const outer = paintedLine(offsetWay(linePoints, outerOffset), BIKE_EDGE_W_M, 0xf2f5ef,
      roadTop + 0.030, { dash: hasTurn });
    const inner = paintedLine(offsetWay(linePoints, innerOffset), BIKE_EDGE_W_M, 0xf2f5ef,
      roadTop + 0.032, { dash: hasTurn });
...
function southWindowTexture(seed) {
  ...
  for (let y = 14; y < 112; y += 34) {
    ...
    for (let x = 12; x < 118; x += 28) {
      ...
      glass.addColorStop(0, lit ? "rgba(255,228,176,0.82)" : "rgba(94,128,148,0.76)");
      glass.addColorStop(1, lit ? "rgba(202,148,82,0.74)" : "rgba(20,32,38,0.88)");
      ctx.fillStyle = glass;
...
function entryTexture(seed, archetype) {
  ...
  if (archetype === "retail" || archetype === "restaurant") {
    ...
    glass.addColorStop(0, "rgba(116,154,172,0.88)");
    glass.addColorStop(1, "rgba(18,28,34,0.96)");
    ctx.fillStyle = glass;
    ...
    glass.addColorStop(0, "rgba(155,188,202,0.88)");
    glass.addColorStop(1, "rgba(32,48,58,0.95)");
    ctx.fillStyle = "rgba(0,0,0,0.28)";
  ...
  for (let i = 1; i < run.length; i += 1) {
    ...
    for (let k = 0; k <= along; k += 1) {
      ...
      for (let j = 0; j <= across; j += 1) {
        ...
        if (known === undefined || top > known) pavedGrid.set(key, top);
      }
    }
  }
}
...
function bikeLaneTexture() {
  ...
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext("2d");
  ctx.fillStyle = "#7fb99b";
  ctx.fillRect(0, 0, size, size);
  for (let i = 0; i < size * size * 0.18; i += 1) {
    const shade = random(i * 19);
    ctx.fillStyle = shade < 0.55 ? "rgba(28,74,58,0.16)" : "rgba(225,246,232,0.13)";
...
function grassTexture() {
  ...
  for (const [colour, count, len] of passes) {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      const angle = (random(n * 7.3) - 0.5) * 1.5 - Math.PI / 2;
...
(async () => {
  ...
  if (yardGeom) {
    addGround(new THREE.Mesh(yardGeom, new THREE.MeshStandardMaterial({
      map: grassTexture(), color: 0xffffff, roughness: 0.98, metalness: 0.0,
      side: THREE.DoubleSide,
    })), "yard");
  }

  const frontWalkGeom = ringGeometry((ground.front_walks || []).map((y) => y.p), YARD_Y + 0.004);
  if (frontWalkGeom) {
  ...
  const serviceGeom = ringGeometry((ground.service_yards || []).map((y) => y.p), YARD_Y + 0.002);
  if (serviceGeom) {
