...
function trimWalkwayForCorners(points, width) {
  ...
  // crossings to bridge the gap, including the synthetic alley-mouth crossings below.
  return trimWay(points, intersectionWalkwayCutback(width));
}
...
function axisStraightCrossingPoints(points) {
  if (!points || points.length < 2) return points || [];
  const a = points[0];
  ...
  const dx = bx - ax;
  const dz = bz - az;
  if (!(Math.hypot(dx, dz) > 1.2)) return [a, b];
  if (Math.abs(dx) >= Math.abs(dz)) {
    const z = (az + bz) / 2;
    return [lonLatFromXZ(ax, z), lonLatFromXZ(bx, z)];
  }
  const x = (ax + bx) / 2;
  return [lonLatFromXZ(x, az), lonLatFromXZ(x, bz)];
}
...
function crossingRoadSpanPoints(points) {
  ...
  const straight = axisStraightCrossingPoints(points);
  const a = straight[0];
...
function addCarriagewayDisk(x, z, radius) {
  const minX = Math.floor((x - radius) / CARRIAGEWAY_CELL);
  const maxX = Math.floor((x + radius) / CARRIAGEWAY_CELL);
  const minZ = Math.floor((z - radius) / CARRIAGEWAY_CELL);
  const maxZ = Math.floor((z + radius) / CARRIAGEWAY_CELL);
  for (let ix = minX; ix <= maxX; ix += 1) {
    for (let iz = minZ; iz <= maxZ; iz += 1) {
      const key = `${ix}:${iz}`;
      let bucket = carriagewayDiskGrid.get(key);
      if (!bucket) carriagewayDiskGrid.set(key, bucket = []);
...
function insideCarriageway(x, z, slack = 0.4) {
  ...
  for (let dx = -1; dx <= 1; dx += 1) {
    for (let dz = -1; dz <= 1; dz += 1) {
      ...
      const disks = carriagewayDiskGrid.get(`${cx + dx}:${cz + dz}`);
      if (disks) {
        for (const [ix, iz, radius] of disks) {
          const limit = Math.max(0, radius - slack);
          if (Math.hypot(x - ix, z - iz) < limit) return true;
        }
      }
    }
  }
...
PAGE_FIELDS = {
    ...
    "crossing_m", "cycleway", "cycleway_both", "cycleway_left", "cycleway_right", "garages",
