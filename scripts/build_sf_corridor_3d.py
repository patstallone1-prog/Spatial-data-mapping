...
function crosswiseCarriagewayAt(x, z, bearing, slack = 0.0) {
...
function bikeLaneZones(lane) {
  ...
  for (let i = 0; i < lane.length; i += 1) {
    ...
    labels.push(crosswiseCarriagewayAt(x, -y, bearing, 0.2) ? "crossing" : "lane");
    if (i) {
...
function addBikeLaneMarkings(way, renderPoints, roadWidth, roadTop) {
  ...
  for (const side of sides) {
    ...
    for (const run of bikeLaneZones(lane)) {
...
function addCarriagewaySegment(ax, az, bx, bz, half) {
  ...
  for (let ix = minX; ix <= maxX; ix += 1) {
    for (let iz = minZ; iz <= maxZ; iz += 1) {
      ...
      bucket.push([ax, az, bx, bz, half, bearing]);
    }
  }
}
...
function addKerbsidePavement(renderPoints, side, inner, walk, color, opacity, y, thickness) {
  ...
  for (const share of [1.0, ...WALK_FALLBACK_WIDTHS_M]) {
    ...
    // The ends are trimmed back so a footway does not run out across the junction it meets.
    // How far back cannot be a fixed distance: `inner + 0.7w` is over seven metres on an
    // ordinary street, and on the thirteen metre stubs between two junctions that is more than
    // the whole way -- so the trim took all of it and the side came out bare. It is capped at a
...
function grassTexture(kind = "yard") {
  if (GRASS_TEXTURES.has(kind)) return GRASS_TEXTURES.get(kind);
  ...
  canvas.width = canvas.height = size;
  const ctx = canvas.getContext("2d");
  const base = kind === "park" ? "#36a846" : "#2f9738";
  const dark = kind === "park" ? "rgba(20,92,29,0.42)" : "rgba(21,80,24,0.46)";
  const light = kind === "park" ? "rgba(111,202,100,0.30)" : "rgba(92,178,78,0.26)";
  ctx.fillStyle = base;
  ctx.fillRect(0, 0, size, size);
  ...
  for (let i = 0; i < 130; i += 1) {
    const r = 10 + random(i * 17 + kind.length) * 54;
    ctx.fillStyle = random(i * 5) < 0.55 ? dark : light;
    ctx.beginPath();
    ctx.arc(random(i * 3) * size, random(i * 7) * size, r, 0, Math.PI * 2);
    ctx.fill();
  }
  for (let i = 0; i < (kind === "park" ? 260 : 520); i += 1) {
    const x = random(i * 19.1 + 11) * size;
    const y = random(i * 23.7 + 5) * size;
    const w = 0.5 + random(i * 3.2) * 1.8;
    const h = 0.5 + random(i * 4.6) * 1.6;
    ctx.fillStyle = random(i * 13.7) < 0.55 ? "rgba(117,92,48,0.20)" : "rgba(221,204,125,0.13)";
    ctx.fillRect(x, y, w, h);
  }
  ...
  for (const [colour, count, len, opacityJitter] of passes) {
    ...
    for (let i = 0; i < count; i += 1) {
      n += 1;
      const x = random(n * 3.1) * size;
      const y = random(n * 5.7) * size;
      const sweep = kind === "park" ? 0.95 : 1.6;
      const angle = (random(n * 7.3) - 0.5) * sweep - Math.PI / 2;
      const l = len + random(n * 11.9) * len;
      ctx.globalAlpha = 0.65 + random(n * 2.6) * opacityJitter * 0.35;
      ctx.beginPath();
      ctx.moveTo(x, y);
  ...
  ctx.strokeStyle = kind === "park" ? "rgba(210,239,170,0.08)" : "rgba(220,231,150,0.055)";
  ctx.lineWidth = 2;
  for (let i = -size; i < size * 2; i += 34) {
    ctx.beginPath();
    ctx.moveTo(i, 0);
    ctx.lineTo(i + size * 0.42, size);
    ctx.stroke();
  }
  ...
  texture.anisotropy = 8;
  texture.colorSpace = THREE.SRGBColorSpace;
  GRASS_TEXTURES.set(kind, texture);
  return texture;
}
...
(async () => {
  ...
  if (parkGeom) {
    addGround(new THREE.Mesh(parkGeom, new THREE.MeshStandardMaterial({
      map: grassTexture("park"), color: 0xffffff, roughness: 0.97, metalness: 0.0,
      side: THREE.DoubleSide,
    })), "park");
  }
  ...
  const grassRings = classifiedGround
    ...
    : (ground.yards || []);
  const yardGeom = ringGeometry(grassRings.map((y) => y.p), YARD_Y);
  if (yardGeom) {
    addGround(new THREE.Mesh(yardGeom, new THREE.MeshStandardMaterial({
      map: grassTexture("yard"), color: 0xffffff, roughness: 0.98, metalness: 0.0,
      side: THREE.DoubleSide,
    })), "yard");
  }
