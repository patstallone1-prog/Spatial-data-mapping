...
function backdropAt(x, z, apron = 0) {
  ...
  const sandy = (apron > 0 && perimeterClassAt(x, z) === 2) || (edge < SEA_SURFACE_M + BEACH_RISE_M && nearBeach(x, z));
  if (apron > 0) {
    ...
    const colour = sandy ? TERRAIN_SAND : TERRAIN_PLATE.clone().lerp(TERRAIN_LAND, Math.exp(-apron / TERRAIN_APRON_COLOUR_M));
...
if (PERIMETER) {
  ...
  const grass = TERRAIN_PLATE, sea = TERRAIN_SEA, sand = TERRAIN_SAND;
      ...
      for (let r = r0; r < r1; r += 1) {
        ...
        while (c < c1) {
          ...
          if (!under) {
            ...
            const col = wet === 1 ? sea : wet === 2 ? sand : grass;
...
window.kerbside = {
  ...
  shoot({ lon, lat, from = 180, tilt = 52, heading = 30, w = 720, h = 405, fov = 45,
          quality = 0.72, targetY = null } = {}) {
    const [x, y] = xy(lon, lat);
