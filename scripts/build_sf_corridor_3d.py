...
//: pavement, the footprint under a building -- is refined coarsely and never for curvature,
//: because nobody sees its chords. A parking lot or a front path is refined for length only:
//: the grid under a multi-storey garage or a flight of steps is a staircase, and chasing every
//: step of it ran a hundred and fifty lots to over a million triangles. A yard or a park, both.
const TERRAIN_REFINE_BY_SURFACE = {
  walk_underlay: [24.0, 0], footprint: [24.0, 0], garage: [24.0, 0],
...
function refineForTerrain(geometry, maxEdge = TERRAIN_REFINE_M, chord = TERRAIN_CHORD_M) {
  ...
  // Keyed by the pair as one number: a string key here was a third of the lift's time.
  ...
  const midpoint = (a, b) => {
    ...
    for (let k = 0; k < arrays.length; k += 1) {
      const n = sizes[k];
      for (let c = 0; c < n; c += 1) arrays[k].push((arrays[k][a * n + c] + arrays[k][b * n + c]) / 2);
    }
    midpoints.set(key, m);
    return m;
  };
...
function addCrossingTactilePads(points, crossingWidth, y) {
  if (!points || points.length < 2) return 0;
  let added = 0;
  if (addTactileWarningPad(points[0], points[1], crossingWidth, y)) added += 1;
  if (addTactileWarningPad(points[points.length - 1], points[points.length - 2],
                           crossingWidth, y)) added += 1;
  return added;
}
...
function cullDetailByDistance(frame, eye = camera) {
  ...
  if (frame % DETAIL_REINDEX_FRAMES === 0) {
    detailTiles = [];
    root.traverse((o) => {
      if (o.isMesh && o.frustumCulled && o.geometry && o.geometry.boundingSphere
          && DETAIL_RANGE_M[o.userData.surface] !== undefined) detailTiles.push(o);
    });
  }
...
window.kerbside = {
  ...
  goTo, setFirstPerson, placeCamera, state, avatar, cullDetailByDistance, DETAIL_RANGE_M, mergeStats,
