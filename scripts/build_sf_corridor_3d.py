...
//: Where the terrain has no data (outside the lidar, under the bay) the ground is flat at 0.
...
if (TERRAIN_FRAME) {
  ...
  const heightAt = (c, r) => cutLiftAt(f.x0 + (c * TERRAIN_MESH_STRIDE + 0.5) * f.step_m,
                                       -(f.y0 + (r * TERRAIN_MESH_STRIDE + 0.5) * f.step_m)) - TERRAIN_MESH_UNDER_M;
  const material = new THREE.MeshStandardMaterial({ color: 0x4a4a45, roughness: 0.99, metalness: 0.0 });
  ...
  for (let r0 = 0; r0 + 1 < rows; r0 += TERRAIN_TILE_VERTICES - 1) {
    for (let c0 = 0; c0 + 1 < cols; c0 += TERRAIN_TILE_VERTICES - 1) {
      ...
      for (let r = 0; r < h; r += 1) {
        for (let c = 0; c < w; c += 1) {
          const i = (r * w + c) * 3;
          position[i] = f.x0 + ((c0 + c) * TERRAIN_MESH_STRIDE + 0.5) * f.step_m;
          position[i + 1] = heightAt(c0 + c, r0 + r);
          position[i + 2] = -(f.y0 + ((r0 + r) * TERRAIN_MESH_STRIDE + 0.5) * f.step_m);
        }
      }
