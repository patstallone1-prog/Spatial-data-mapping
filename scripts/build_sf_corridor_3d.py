...
function indexOfficialCurbGeometry(lines) {
  ...
  for (const line of lines || []) {
    ...
    const grid = line.r === "island" ? officialIslandCurbGrid : officialCurbGrid;
...
//: only. The other side is the median, and the other half of the road is beyond it.
function isDividedHalf(way) {
  return Boolean(way.oneway || way.osm_oneway) && Array.isArray(way.walk_sides)
    && way.walk_sides.length === 1;
}
...
function envelopeEdgesAt(x, z, nx, nz, way, reach) {
  ...
  if (isDividedHalf(way)) {
    ...
    const island = side > 0 ? islands.filter((v) => v < kerb - 2.5 && v > -12).pop()
      : islands.find((v) => v > kerb + 2.5 && v < 12);
  ...
  if (plausible(a, b, 3.0) && b - a >= 4.2 && (a !== neg(kerbs) || b !== pos(kerbs))) return [a, b];
