...
function sideBlockedByCarriageway(renderPoints, side, inner) {
  // Fallback pavement belongs outside the street, not in the slot between two divided
  // carriageways. Sample just beyond the kerb line: if that strip is inside some other
  // carriageway, drawing sidewalk tiles there makes the middle of the road look paved.
  const probe = inner + 1.8;
  let sampled = 0;
  let blocked = 0;
  for (let i = 0; i < renderPoints.length; i += Math.max(1, Math.floor(renderPoints.length / 7))) {
    const centre = offsetWay([renderPoints[Math.max(0, i - 1)], renderPoints[i],
                              renderPoints[Math.min(renderPoints.length - 1, i + 1)]],
                             side * probe)[1];
    const [x, y] = xy(centre[0], centre[1]);
    sampled += 1;
    if (insideCarriageway(x, -y, 0.15)) blocked += 1;
  }
  return sampled > 0 && blocked >= sampled * 0.45;
}
...
function walkSidesToDraw(way, renderPoints, inner) {
  ...
  for (const side of [1, -1]) {
    if (sideBlockedByCarriageway(renderPoints, side, inner)) continue;
    if (stated === undefined) {
      drawn.push(side);
      continue;
    }
...
function addKerbsidePavement(renderPoints, side, inner, walk, color, opacity, y, thickness) {
  ...
  const emit = (from, to) => {
    ...
    laid += addPavementRibbon(offsetWay(stretch, side * (inner + width / 2)), width,
...
function addPropertyLinePavementUnderlay(renderPoints, side, inner, walk, color, opacity) {
  const wanted = wayLength(renderPoints);
  if (!(wanted > 0)) return 0;
  const width = Math.max(PROPERTY_LINE_PAVEMENT_WIDTH_M, walk);
  const back = Math.min(Math.max(1.6, inner + 2.4), wanted * 0.32);
  const [cutStart, cutEnd] = kerbsideTrims(renderPoints, back);
  const spine = densifyWay(trimWayEnds(renderPoints, cutStart, cutEnd), 2.0);
  if (spine.length < 2) return 0;
  let laid = 0;
  const centre = offsetWay(spine, side * (inner + width / 2));
  for (const run of pavementRunsOutsideCarriageway(centre, width)) {
...
const TACTILE_PAD_DEPTH_M = 0.72;
...
const GROUND_STACK = {
  YARD_Y, SERVICE_YARD_Y, PARK_Y, FRONT_WALK_Y, COURT_Y, COURT_LINE_Y,
};
{
  const seen = new Map();
...
function addOfficialCurbZoneBands(records) {
  // Kerb paint, on the kerb.
  //
  // San Francisco paints the top of the kerb and its face, for the length of the zone, and the
  // colour is the policy: red no stopping, yellow commercial loading, white passenger loading,
  // green short-term, blue accessible. This drew the published policy line flat on the roadway
  // wherever that line happened to fall, which is not where this model's kerbs are -- so bands
  // ended up out in traffic and up on the pavement. Now each one is snapped to the kerb and
  // painted the way kerb paint is painted: a band over the top and a face down to the road.
  const buckets = new Map();
  const materials = new Map();
  const materialFor = (color) => {
    if (!materials.has(color)) {
      materials.set(color, new THREE.MeshStandardMaterial({
        color, roughness: 0.84, metalness: 0.0, side: THREE.DoubleSide,
        polygonOffset: true, polygonOffsetFactor: -8, polygonOffsetUnits: -16,
      }));
    }
    return materials.get(color);
  };
  const bucketFor = (colorName, color) => {
    const key = `${colorName}:${color}`;
    if (!buckets.has(key)) buckets.set(key, { colorName, color, positions: [], index: [] });
    return buckets.get(key);
  };
  const top = roadSurfaceTop() + 0.004;
  const face = ROAD_TOP_M + 0.004;
  let added = 0;
  let unplaced = 0;
  for (const record of records) {
    const color = curbZoneColor(record);
    if (!color || !Array.isArray(record.lines)) continue;
    const bucket = bucketFor(record.curb_color, color);
    ...
    for (const run of runs) {
      for (let i = 1; i < run.length; i += 1) {
        ...
        // Over the top of the kerb, inward from its edge onto the pavement.
        let base = bucket.positions.length / 3;
        bucket.positions.push(
          a.x, top, a.z,
          a.x + a.nx * CURB_BAND_W_M, top, a.z + a.nz * CURB_BAND_W_M,
          b.x + b.nx * CURB_BAND_W_M, top, b.z + b.nz * CURB_BAND_W_M,
          b.x, top, b.z,
        );
        bucket.index.push(base, base + 1, base + 2, base, base + 2, base + 3);
        // And down the face of the kerb to the gutter, which is the part you see from a car.
  ...
  for (const bucket of buckets.values()) {
    ...
    geometry.computeVertexNormals();
    const mesh = new THREE.Mesh(geometry, materialFor(bucket.color));
    mesh.userData = {
      surface: `official_curb_zone:${bucket.colorName}`,
      source: "sfmta_curb_zones",
      provenance: "official_digital_curb_policy_polyline_snapped_to_rendered_kerb",
    };
    groups.furniture.add(mesh);
  }
  return { segments: added, unplaced };
}
...
const AVATAR_HEIGHT = 2.13;    // just over human height: about seven feet for the demo avatar
