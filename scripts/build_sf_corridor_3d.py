...
function roofTexture(seed, base, archetype) {
  ...
  if (archetype === "residential" || archetype === "generic") {
    if (random(seed + 71) < 0.38) {
      // Solar modules, dark blue-black with thin metal mullions.
      const x0 = 12 + Math.floor(random(seed + 72) * 36);
      const y0 = 14 + Math.floor(random(seed + 73) * 42);
      ctx.fillStyle = "rgba(12,28,42,0.92)";
      ctx.fillRect(x0, y0, 50, 28);
      ctx.strokeStyle = "rgba(125,154,170,0.58)";
      for (let x = x0 + 10; x < x0 + 50; x += 10) { ctx.beginPath(); ctx.moveTo(x, y0); ctx.lineTo(x, y0 + 28); ctx.stroke(); }
      for (let y = y0 + 9; y < y0 + 28; y += 9) { ctx.beginPath(); ctx.moveTo(x0, y); ctx.lineTo(x0 + 50, y); ctx.stroke(); }
    } else if (random(seed + 81) < 0.28) {
      // Roof deck: plank field, pale parapet, a couple of planters.
      ctx.fillStyle = "rgba(126,96,68,0.58)";
      ctx.fillRect(18, 18, 54, 34);
      ctx.strokeStyle = "rgba(60,45,34,0.42)";
      for (let y = 22; y < 52; y += 6) { ctx.beginPath(); ctx.moveTo(18, y); ctx.lineTo(72, y); ctx.stroke(); }
      ctx.strokeStyle = "rgba(230,226,214,0.70)";
      ctx.strokeRect(17.5, 17.5, 55, 35);
      ctx.fillStyle = "rgba(55,92,50,0.72)";
      ctx.fillRect(23, 23, 8, 5);
      ctx.fillRect(58, 42, 8, 5);
    }
  } else if (archetype === "office" || archetype === "hotel" || archetype === "industrial") {
    ctx.fillStyle = "rgba(35,39,40,0.35)";
    for (let i = 0; i < 3; i += 1) {
      const x = 18 + random(seed + i * 5) * 72;
      const y = 16 + random(seed + i * 7) * 74;
...
function buildFences(ground) {
  ...
  for (const kind of Object.keys(fences)) {
    ...
    for (let i = 0; i + 3 < flat.length; i += 4) {
      ...
      // A last check against the one authority on where the road is. The build already tested
      // these against the road band, and this costs nothing, and between the two of them a
      // fence across a carriageway cannot survive.
    ...
    groups.ground.add(new THREE.Mesh(geom, fenceMaterial(style)));
  }
}
...
(async () => {
  ...
  if (frontWalkGeom) {
    groups.ground.add(new THREE.Mesh(frontWalkGeom, new THREE.MeshStandardMaterial({
  ...
  if (serviceGeom) {
    groups.ground.add(new THREE.Mesh(serviceGeom, new THREE.MeshStandardMaterial({
